...
export function AdminPage() {
    // ✅ ALL HOOKS MUST BE DECLARED AT TOP LEVEL — NO CONDITIONALS ABOVE
    ...
    // ✅ Blog posts query (now unconditional)
    const {
        data: posts = [],
        ...
        enabled: !!session?.authenticated, // only fetch if authenticated
    });

    // ✅ Page content queries (unconditional)
    const {
        data: pageContents = [],
        isLoading: isPageContentsLoading,
        refetch: refetchPageContents,
    } = useAllPageContent({ enabled: !!session?.authenticated,});

    // ✅ Admin logs query (unconditional)
    const {
        data: logs = [],
    ...
    // ✅ Side effect: refetch data when session changes
    useEffect(() => {
    ...
    }, [session?.authenticated, refetchPosts, refetchPageContents, refetchLogs]);
    ...
    const handleUpdatePage = (
        ...
        updatePageContentMutation.mutate(
            { country, pageType, language, data },
            {
                onSuccess: () => {
    ...
    const handleDeletePage = (country: Country, pageType: PageType, language: Language) => {
        if (confirm("Are you sure you want to delete this page content?")) {
            deletePageContentMutation.mutate({ country, pageType, language }, {
                onSuccess: () => {
    ...
    // ✅ Now render main admin UI — all hooks already called!
    return (
        <div className="min-h-screen bg-background">
            <div className="container mx-auto px-4 py-8">
                <div className="mb-8 flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold text-foreground py-20">Admin Panel</h1>
                ...
                <Tabs defaultValue="blog-posts" className="w-full">
                            ...
                            <Dialog open={!!editingPost} onOpenChange={() => setEditingPost(null)}>
                                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                                    ...
                                    <BlogPostForm
                                        ...
                                        onSubmit={(data) =>
                                            updatePostMutation.mutate({ slug: editingPost.slug, data })
                                        }
                        ...
                        {isPageContentsLoading ? (
                            ...
                            <div className="space-y-4">
                                {pageContents.map((content) => (
                                    <Card
                                        key={`${content.country}-${content.pageType}-${content.language}`}
                                        className="relative"
                                    >
                                        <CardHeader>
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex gap-2 ml-4">
                                                    <Button
                                                        ...
                                                        data-testid={`button-edit-${content.country}-${content.pageType}-${content.language}`}
                                                    >
                                                        <Edit className="w-4 h-4" />
                                                    </Button>
                                                    <Button
                                                        ...
                                                        onClick={() =>
                                                            handleDeletePage(
                                                                content.country as Country,
                                                                content.pageType as PageType,
                            ...
                            <Dialog open={!!editingPageContent} onOpenChange={() => setEditingPageContent(null)}>
                                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>Update the page content details</DialogDescription>
                                    </DialogHeader>
                                    <PageContentForm
                                        defaultValues={editingPageContent}
                                        onSubmit={(data) =>
                                            updatePageContentMutation.mutate({
                                                country: editingPageContent.country as Country,
                                                pageType: editingPageContent.pageType as PageType,
                                                language: editingPageContent.language as Language,
                                                data,
                                            })
                                        }
                    ...
                    <TabsContent value="admin-logs" className="space-y-6">
                        <AdminLogsSection logs={logs} isLoading={logsLoading} />
                    </TabsContent>
                </Tabs>
            </div>
        </div>
...
// ✅ Updated AdminLogsSection to be pure (no hooks)
interface AdminLogsSectionProps {
    logs: any[];
    isLoading: boolean;
}

function AdminLogsSection({ logs, isLoading }: AdminLogsSectionProps) {
    if (isLoading) {
        return (
            <div className="space-y-4">
                {[...Array(5)].map((_, i) => (
...
// ✅ Keep BlogPostForm and PageContentForm unchanged (they’re fine)
